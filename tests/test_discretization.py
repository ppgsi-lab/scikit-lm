...
def test_imputer_discretized_scores_numeric_generates_categorical() -> None:
    ...
    assert out.loc[4, "n"] == 15.0  # scored: mean(10, 20)
    assert out.loc[4, "c"] == "z"  # generated
...
def test_imputer_discretized_routes_score_and_generate() -> None:
    ...
    LanguageModelImputer(backend=off).fit_transform(X)  # discretization off (default)
    assert off.generate_batches and not off.score_batches
...
def test_imputer_discretization_map_non_numeric_column_raises() -> None:
    X = pd.DataFrame({"a": [1.0, 2.0, np.nan], "c": ["x", "y", "x"]})
    imp = LanguageModelImputer(
        backend=FakeBackend(), discretization={"c": DiscretizationConfig(bins=2)}
    )
...
def test_imputer_discretized_generation_failure_raises_runtime_error() -> None:
    ...
    with pytest.raises(RuntimeError, match="generation failed"):
        imp.transform(X)  # the categorical cell never decodes -> loud failure
